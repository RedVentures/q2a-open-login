			...
			'providers' => array(
				$provider => array(
					...
					'keys' => array(
						...
						'tenant' => $provider === 'Auth0' ? qa_opt("{$key}_tenant") : null,
		...
		foreach ($allProviders as $providerFile) {
			...
			if ($provider === 'Auth0') {
				$form['fields'][] = array(
					...
					'value' => qa_html(qa_opt("{$key}_tenant")),
					'tags' => "NAME=\"{$key}_tenant_field\"",
				);

				$form['fields'][] = array(
					'label' => $provider . ' Audience:',
					'value' => qa_html(qa_opt("{$key}_audience")),
					'tags' => "NAME=\"{$key}_audience_field\"",
				);
			}
